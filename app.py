    ...
    with st.container(key="main_box_right", 
        ...
        def save_popup(p_hvis, p_hir, p_hfused, p_d): 
            ...
            if choice == "Create a new file":
                ...
                vis_path = fr"image_assets\vis_img\{st.session_state.get('vis_file_path')}" 
                ir_path = fr"image_assets\IR_img\{ st.session_state.get('IR_file_path')}" 
                fused_path = "fused.png"

                # 2. Pass the clean string paths directly to your PDF maker
                creatsv.makepdf(vis_path, ir_path, fused_path, p_hvis, p_hir, p_hfused, p_d, name_of_file)
                ...
                ir_path = fr"image_assets\IR_img\{ st.session_state.get('IR_file_path')}"
                fused_path = "fused.png" 
                creatsv.makepdf(vis_path, ir_path, fused_path, p_hvis, p_hir, p_hfused, p_d, choice)
