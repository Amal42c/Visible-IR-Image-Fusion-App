    ...
    with st.container(key="main_box_right", 
        ...
        def save_popup(p_hvis, p_hir, p_hfused, p_d): 
            ...
            if choice == "Create a new file":
                ...
                pdf = creatsv.makepdf(vis_path, ir_path, fused_path, p_hvis, p_hir, p_hfused, p_d)
                st.download_button(
                ...
                ir_path = fr"image_assets\IR_img\{ st.session_state.get('IR_file_path')}"
                fused_path = "fused.png" 
                pdf = creatsv.makepdf(vis_path, ir_path, fused_path, p_hvis, p_hir, p_hfused, p_d, choice)
                st.download_button(
                    label="Download Fusion Report in .pdf",
                    data=pdf,
                    file_name=choice,
